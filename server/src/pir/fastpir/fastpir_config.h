...
static_assert(std::popcount(POLY_MODULUS_DEGREE) == 1, "POLY_MODULUS_DEGREE must be a power of 2");

constexpr uint64_t PRIME_54 = 18'014'398'509'309'953ULL;
// static_assert(is_prime(PRIME_54), "PRIME_54 must be prime"); // seems unnecessary to code this up just to check this. VERIFY MANUALLY.

constexpr uint64_t PRIME_55 = 36'028'797'018'652'673ULL;
// static_assert(is_prime(PRIME_55), "PRIME_55 must be prime"); // seems unnecessary to code this up just to check this. VERIFY MANUALLY.
...
static_assert(PLAIN_MODULUS >= 1ULL << PLAIN_BITS, "PLAIN_MODULUS must be greater than 2^PLAIN_BITS");
static_assert(PLAIN_MODULUS % (2 * POLY_MODULUS_DEGREE) == 1, "PLAIN_MODULUS must be congruent to 1 modulo 2*POLY_MODULUS_DEGREE");
// static_assert(is_prime(PLAIN_MODULUS), "PLAIN_MODULUS must be prime"); // seems unnecessary to code this up just to check this. VERIFY MANUALLY.
