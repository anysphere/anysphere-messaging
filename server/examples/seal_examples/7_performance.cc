...
void bfv_performance_test(SEALContext context) {
  ...
  */
  ...
  for (size_t i = 0; i < static_cast<size_t>(count); i++) {
    ...
    */
    size_t buf_size =
        static_cast<size_t>(encrypted.save_size(compr_mode_type::none));
  ...
  auto avg_rotate_rows_one_step =
      time_rotate_rows_one_step_sum.count() / (2 * count);
  ...
  if (context.using_keyswitching()) {
    ...
    cout << "Average rotate rows random: " << avg_rotate_rows_random
         << " microseconds" << endl;
    cout << "Average rotate columns: " << avg_rotate_columns << " microseconds"
         << endl;
  }
  cout << "Average serialize ciphertext: " << avg_serialize << " microseconds"
       << endl;
#ifdef SEAL_USE_ZLIB
  cout << "Average compressed (ZLIB) serialize ciphertext: "
       << avg_serialize_zlib << " microseconds" << endl;
#endif
#ifdef SEAL_USE_ZSTD
  cout << "Average compressed (Zstandard) serialize ciphertext: "
       << avg_serialize_zstd << " microseconds" << endl;
#endif
  cout.flush();
}
  ...
  */
    ...
    */
    ...
    if (context.using_keyswitching()) {
      ...
      */
      ...
      int random_rotation =
          static_cast<int>(rd() & (ckks_encoder.slot_count() - 1));
      time_start = chrono::high_resolution_clock::now();
  ...
  if (context.using_keyswitching()) {
    ...
    cout << "Average rotate vector random: " << avg_rotate_random
         << " microseconds" << endl;
    cout << "Average complex conjugate: " << avg_conjugate << " microseconds"
         << endl;
  }
  cout << "Average serialize ciphertext: " << avg_serialize << " microseconds"
       << endl;
