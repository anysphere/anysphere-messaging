...
\begin{enumerate}
    ...
    \item Let $\{\reg_1, \cdots, \reg_k\}$ be the keys of $\frienddb$, with $k\leq B$. Construct $S = [\reg_1,\cdots, \reg_k, \reg, \cdots, \reg]$, where we add $B - k$ copies of $\reg$,a dummy registration info with $\reg[0] = 0$ and $\reg[1]$ a randomly generated public key. Sample $\reg_s, \reg_r$ uniformly and independently at random from $S$. 
