...
Let $N$ be the number of honest clients, and take $R = 2N(\lambda)T(\lambda)$. Let $\Sim_{\sym}$ and $\Sim_{\pir}$ be the simulators defined in \cref{defn:sym-security} and \cref{defn:pir-security} respectively, with parameters $N$ and $R$. At step (2), (4.a), and (4.c) of \cref{defn:messaging-ideal-world-experiment}, our simulator will run modified versions of the corresponding client methods in the respective steps of \cref{defn:messaging-real-world-experiment} for each $i \in \cH$. The important modifications involve replacing all lines that involve unleaked information.
