...
Given a series of secret keys $\{kx_i^S\}_i$, let $\KXE(\{kx_i^S\}, \cdot)$ output $\{\KXE(kx_i^S, \cdot)\}_i$.
\end{definition}
\begin{definition}[ACK Oracle]
Given a key exchange secret key $kx^S$, the ACK oracle $O_\ack(kx^S, \cdot)$ takes as input a ``fake key" $k$ and a set of ciphertexts $\ct_i$, computes
$$sk \leftarrow \KX(1^{\lambda}, k, kx^S)$$
$$m_i \leftarrow \dec(sk, \ct_i).$$
let $\ell$ be the number of $m_i$ that are not $\bot$. The oracle output $\enc(sk, \ell)$.

Given a series of secret keys $\{kx_i^S\}_i$, let $O_{\ACK}(\{kx_i^S\}, \cdot)$ output $\{O_{\ACK}(kx_i^S, \cdot)\}_i$.
\end{definition}
\begin{definition}
