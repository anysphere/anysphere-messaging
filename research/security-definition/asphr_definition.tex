...
\begin{enumerate}
    ...
    \item We use a key-private IND-CPA symmetric key AEscheme $\Pi_{\sym}$.
    \item We use a PIR protocol $\Pi_{\pir}$.
\end{enumerate}
\textbf{Remark}: Recall that the PIR protocol takes the length of each database entry as a parameter. Let $L_{\msglb}$ denote the length of the labeled message, and let $L_{\ct}$ be the length of the ciphertext generated by encoding $\msg^{\lb}$ with $\Pi_{\sym}.$. 
