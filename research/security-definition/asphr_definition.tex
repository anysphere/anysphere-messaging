...
\end{enumerate}
then $sk_{AB} = sk_{BA}$ with probability $1$. \todo{This simplifies the security definition below. Otherwise, we need to give adversary power to call $\Enc$ on $\sk_{ij}$, where $h$ is uncompromised.} For any plaintext $m$, we have
...
\begin{definition}[KXM Oracle]
Given a key exchange secret key $kx^S$, the KXM(key exchange and message) oracle $\KXM(kx^S, \cdot)$ takes as input a ``fake key" $k$ and a message $m$, computes
$$sk \leftarrow \KX(1^{\lambda}, k, kx^S)$$
and outputs $\enc(sk, m)$.

Given a series of secret keys $\{kx_i^S\}_i$, let $\KXM(\{kx_i^S\}, \cdot)$ output $\{\KXM(kx_i^S, \cdot)\}_i$.
\end{definition}
...
\begin{enumerate}
    ...
    \begin{enumerate}
        \item $i, j, m \leftarrow \cA^{\KXM(\{kx_i^S\}, \cdot)}().$
...
\begin{enumerate}
    \item For $i$ from $1$ to $N$, $(kx_i^P, kx_i^S) \leftarrow \gen(1^{\lambda}).$
    \item For $(i, j)$ in $[N]^2$, $sk_{ij} \leftarrow \KX(1^{\lambda}, kx_i^P, kx_j^S)$.
    \item $\cA$ remembers $1^{\lambda}, r, \{kx_i^P\}_{i \in [N]})$.
    \item For $r$ from $1$ to $R$
    \begin{enumerate}
        \item $i, j, m \leftarrow \cA^{\KXM(\{kx_i^S\}, \cdot)}()$.
...
Then we say the encryption scheme is key-private IND-CPA if there exists a p.p.t simulator $\Sim$ such that for any p.p.t adversary with oracle $\cA^O$, the view of $\cA$ under the real world experiment and the ideal world experiment are computationally indistinguishable. The view of $\cA$ is defined as the input, output, and random coins of $\cA$, plus the array $\{ct_r\}_{r \in [R]}$.
\end{definition}
