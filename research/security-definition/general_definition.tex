...
$$\mathsf{Leak}(\{\cI_{i, t}\}, \cK) = (L_f, L_m)$$
where
$$L_f = \{(i, \reg_j, t): j \in \cK, \trust(\reg_j) = \cI_{i, t}\}.$$
$$L_m = \{(i, \reg_j, \msg, t): j \in \cK, \mathsf{SendMessage}(\reg_j, \msg) = \cI_{i, t}\}.$$
\end{definition}
...
Finally, we require the messaging scheme to satisfy Integrity: while a malicious server can drop messages between users, it shouldn't be able to forge messages between honest users. We define the notion of integrity rigorously.
\begin{definition}[Integrity]
Same notation as before. Consider the real world experiment described in \cref{defn:real-world}. Take any pair of honest users $i, j \in \cH$, and consider the view of $j$ at the end of the experiment obtained by calling $V_j = (\cF, \cM) \leftarrow C_j.\mathsf{GetView}()$.  Then we say the messaging scheme guarantees Integrity if 
\begin{multline*}
 \{\msg : (\reg_i, \msg) \subset \cM\} \subset \{\msg : \exists i, \send(\reg_j, \msg) = \cI_{i, t}\}.   
\end{multline*}
\end{definition}