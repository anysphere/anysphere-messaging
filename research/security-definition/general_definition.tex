...
\todo{SIM-security is a relatively novel form of security defined in NIAR. Because it is relatively unusual, I'm defining a IND-security below.}

\begin{figure}
\begin{framed}
\begin{definition}[Real world IND Experiment] \hfill\\
\label{defn:real-world}
Same notation as above. The real-world IND-experiment with adversary $\mathsf{Real-IND}^{\cA}(1^{\lambda})$ is described below, parametrized by bit $b \in \{0, 1\}$.
\begin{enumerate}
\item $N, T, \cK, \cH \leftarrow \cA(1^{\lambda})$.
\item For each $i \in \cH$, $\reg_i \leftarrow C_i.\mathsf{Register}(1^{\lambda}, i, N)$. 
